...
def inference(net, state_dict_path, test_dl, test_metadata_df, loss_fn, label_path, k=1):
    with open(label_path, 'r') as infile:
        data = json.load(infile)

    torch.cuda.empty_cache()
    device = "cuda" if torch.cuda.is_available() else 'cpu'
    # state_dict = "effnet_baseline/15_crop-15e_noiseAug/checkpoints/epoch=9-step=550.ckpt"
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(test_dl):
                ...
                with torch.cuda.amp.autocast():
                    ...
                    _loss += [loss_fn(out, batch['labels']).cpu().numpy()]
                    _preds += [out.cpu().numpy()]
            preds.append(np.vstack(_preds))
            loss.append(np.vstack(_loss))
