...
def inference_k_random(net, state_dict_path, test_dl, test_metadata_df, loss_fn, label_path, k=1):
    ...
    #print(preds[0][555])
    #print('done')
    #print(f'Shape preds: {np.shape(preds)}')
    #preds_norm = []
    
    #for i in range(len(preds[0])):
        #preds[0][i] = normalize([preds[0][i]])
        #preds[0][i] = normalize([preds[0][i]], norm="l1")
        #preds[0][i] = normalize([preds[0][i]], norm="max")
        
    #print(f'Shape preds_norm: {np.shape(preds_norm)}')
    #print(np.shape(preds[0][0]))
    #test_preds = normalize([preds[0][0]])
    #print(test_preds)
    preds = np.mean(preds, axis=0)
    #preds = np.mean(normalize([preds]), axis=0)
    #preds = np.mean(normalize([preds], norm="l1"), axis=0)
    #preds = np.mean(normalize([preds], norm="max"), axis=0)
